...
d3.csv("data/car_clean.csv").then(data => {
  ...
  const totalAverage = d3.mean(cleanData, d => d.Price);

  // Group and roll up to calculate average price per brand (sort descending)
  const raw = d3.rollups(cleanData, v => d3.mean(v, d => d.Price), d => d.Brand)
  ...
  const xScale = d3.scaleLinear()
    .domain([0, xMax * 1.18]) // Space on right for price label
    .range([0, innerW]);
